...
class DisplaysControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create display" do
    assert_difference('Display.count') do
      post displays_url, params: { display: { description: @display.description, title: @display.title } }
    end
  ...
  test "should update display" do
    patch display_url(@display), params: { display: { description: @display.description, title: @display.title } }
